...
# hdfs_a = 'hdfs://localhost:9000'
# model_path = hdfs_a + "/user/hadoop/model"
...
# # # HDFS에 저장된 CSV 파일 경로
# # hdfs_host = "172.17.0.4"  # 하둡 도커 컨테이너의 IP 주소
# # hdfs_port = "9866"          # HDFS 포트 (일반적으로 8020)
# hdfs_path = "/user/hadoop/data/action_data.csv"

# # # HDFS 접근 주소 설정
# # hdfs_address = f"hdfs://{hdfs_host}:{hdfs_port}"
...
#     df = spark.read.text(hdfs_a + "/test.txt")
#     df.show(truncate=False)

# def read_hadoop_file(hdfs_host, hdfs_port, hdfs_path):
#     # HDFS 호스트와 포트로 연결된 클라이언트 생성
...
#     data = spark.read.text(hdfs_a + "/text/txt")
#     print(data.show())
#     #to_load_csv(spark)
...
#     # try:
#     #     # HDFS에서 CSV 파일 읽기
#     #     df = spark.read.csv(f"{hdfs_address}{hdfs_path}", header=True, inferSchema=True)
...
#     #     .master("spark://localhost:7077").config("fs.defaultFS", hdfs_path).getOrCreate()
