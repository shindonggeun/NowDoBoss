...
for k in range(2, 25):  # 군집 개수를 2부터 10까지 시도
...
# 각 업종별 매출을 비율로 변환
df['RESTAURANT'] = df['RESTAURANT'] / df['당월_매출_금액_x'] * 100
df['ACADEMY'] = df['ACADEMY'] / df['당월_매출_금액_x'] * 100
df['LEISURE'] = df['LEISURE'] / df['당월_매출_금액_x'] * 100
df['SERVICE'] = df['SERVICE'] / df['당월_매출_금액_x'] * 100
df['RETAIL'] = df['RETAIL'] / df['당월_매출_금액_x'] * 100
df['HOUSEHOLDS'] = df['HOUSEHOLDS'] / df['당월_매출_금액_x'] * 100
# 결과 확인
print(df)
...
# '자치구코드' 열을 제외한 열을 군집화에 사용
X = df.drop(columns=['행정동_코드'])

# 최적의 군집 개수를 찾기 위한 반복문
best_score = -1
best_k = 0

for k in range(3, 10):  # 군집 개수를 2부터 10까지 시도
    kmeans = KMeans(n_clusters=k, random_state=42)
    cluster_labels = kmeans.fit_predict(X)
    silhouette_avg = silhouette_score(X, cluster_labels)
