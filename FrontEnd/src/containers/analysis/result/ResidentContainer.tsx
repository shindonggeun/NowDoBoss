...
const ResidentContainer = forwardRef((_, ref: Ref<HTMLDivElement>) => {
  ...
  const { data: ResidentPopulationData, status: residentPopulationStatus } =
    useQuery({
      ...
      queryFn: () =>
        getResidentPopulationData(String(selectedCommercial.code), periodCode),
...
ResidentContainer.displayName = 'ResidentPopulationAnalysisContainer' // eslint 에 위반되는 조건 해결
